...
@php
    ...
    // Bundesweit unzulässige Buchstabenkombinationen (NS-Bezug) – nie vorschlagen.
    $verboten = ['HJ', 'KZ', 'NS', 'SA', 'SD', 'SS'];
    $kombis = collect(config('kennzeichen_woerter', []))
        ...
        ->reject(fn ($rest) => in_array($rest, $verboten, true))
...
@endphp
    @once
        ...
        <style>
            /* Layout-Korrekturen für die Kennzeichen-Eingabe (Button rutschte übers Schild,
               Labels nicht unter den Segmenten). */
            /* Button NEBEN dem Schild; bei wenig Platz umbrechen. */
            #generator .nt-row{flex-direction:row;align-items:center;gap:18px;flex-wrap:wrap;margin-bottom:26px}
            /* Container auf die tatsächliche Schildbreite schrumpfen, damit die
               Labels darunter exakt zu den Segmenten passen und der Button
               direkt daneben sitzt (kein Überlappen). */
            #generator .licenseplate-container{width:fit-content;max-width:100%}
            #generator .nt-row>.cta{margin:0;flex:0 0 auto}
            /* Labels exakt unter die Segmente (Spaltenraster = Schild-Segmente). */
            #generator .licenseplate-labels{display:grid;grid-template-columns:10.8% 21% 17% 15% auto;width:100%;margin:0;gap:0}
            #generator .licenseplate-labels label{margin:0!important;text-align:center;font-size:12px;color:var(--mut)}
            #generator .nt-licenseplate-label--symbol{grid-column:2}
            #generator .nt-licenseplate-label--letters{grid-column:4}
            #generator .nt-licenseplate-label--numbers{grid-column:5}
        </style>
    @endonce
